"""Internal edit implementation moved under the canonical edit core."""
...
def run_make(
    ...
    """
    from .service import make_workbook as _service_make_workbook
    ...
    return cast(PatchResult, _service_make_workbook(cast(Any, resolved_request)))
...
def run_patch(
    ...
    """
    from .service import patch_workbook as _service_patch_workbook
    ...
    if policy is not None:
        resolved_request = request.model_copy(
            update={
                ...
                "out_dir": (
                    policy.ensure_allowed(request.out_dir)
                    if request.out_dir is not None
                    else None
                ),
            }
        )
    return cast(PatchResult, _service_patch_workbook(cast(Any, resolved_request)))
...
def _resolve_input_path(path: Path, *, policy: PathPolicyProtocol | None) -> Path:
...
def _resolve_make_output_path(path: Path, *, policy: PathPolicyProtocol | None) -> Path:
