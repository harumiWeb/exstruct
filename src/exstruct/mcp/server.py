...
def _normalize_patch_op_aliases(op_data: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    Args:
        op_data: Raw patch operation payload.
        index: Source index in the ops list.

    Returns:
        Normalized patch operation payload.
...
def _alias_to_canonical_with_conflict_check(
    ...
    *,
    ...
    Args:
        ...
        op_name: Operation type that allows this alias.

    Raises:
        ValueError: If alias conflicts with canonical field.
    """
...
def _normalize_dimension_size_aliases(op_data: dict[str, Any], *, index: int) -> None:
    """Normalize width/height aliases for set_dimensions operation.
    ...
    Raises:
        ValueError: If aliases conflict with canonical fields.
    """
    if op_data.get("op") != "set_dimensions":
        return
    _alias_to_canonical_with_conflict_check(
        op_data,
        index=index,
        alias="height",
        canonical="row_height",
        op_name="set_dimensions",
    )
    _alias_to_canonical_with_conflict_check(
        op_data,
...
def _normalize_draw_grid_border_range(op_data: dict[str, Any], *, index: int) -> None:
    ...
    Raises:
        ValueError: If shorthand conflicts with explicit fields.
    """
    ...
    start, row_count, col_count = _parse_a1_range_geometry(range_ref, index=index)
...
def _parse_a1_range_geometry(range_ref: str, *, index: int) -> tuple[str, int, int]:
    """Parse A1 range and return top-left cell + (rows, cols).

    Args:
        range_ref: A1 range string.
        index: Source index in ops.

    Returns:
        Tuple of (base_cell, row_count, col_count).

    Raises:
        ValueError: If range format is invalid.
    """
    candidate = range_ref.strip().upper()
    if ":" not in candidate:
        raise ValueError(
            _build_patch_op_error_message(
    ...
    start_ref, end_ref = candidate.split(":", maxsplit=1)
    start_col, start_row = _split_a1_cell(start_ref, index=index)
    end_col, end_row = _split_a1_cell(end_ref, index=index)
    min_col = min(start_col, end_col)
    max_col = max(start_col, end_col)
    min_row = min(start_row, end_row)
    max_row = max(start_row, end_row)
    return (
        f"{_column_index_to_label(min_col)}{min_row}",
        max_row - min_row + 1,
        max_col - min_col + 1,
    )


def _split_a1_cell(cell_ref: str, *, index: int) -> tuple[int, int]:
    """Parse single A1 cell into numeric column/row.

    Args:
        cell_ref: A1-style cell reference.
        index: Source index in ops.

    Returns:
        Tuple of (column_index, row_index), both 1-based.
