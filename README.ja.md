...
- `exstruct_patch` はデザイン編集op（`draw_grid_border` / `set_bold` / `set_fill_color` / `set_dimensions`）と、逆操作用の `restore_design_snapshot` をサポートします。デザイン編集opはopenpyxl経路で処理されるため、`.xls` 入力ではエラーになります（`.xlsx`/`.xlsm` に変換して利用してください）。
